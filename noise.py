...
def retrieve_passage_with_metrics(query, documents, embeddings, top_k=3):
    ...
    for result in search_results:
        ...
        relevance_score = 1 if similarity_score > 0.4 else 0
...
output_folder = "results_0.4_temperature/results_cohere/crosslingual/moderate_errors"
...
noisy_queries_file = "queries/noisy_queries_moderate_english/noisy_queries_moderate_english.txt"  # Adjust path if necessary
noisy_queries = load_noisy_queries(noisy_queries_file)
...
# Save to CSV inside "results" folder
output_folder = "results_0.4_temperature/results_labse/monolingual/moderate_errors"
os.makedirs(output_folder, exist_ok=True)  # Create folder if it doesn't exist

output_csv = os.path.join(output_folder, "english_english_noisy_queries_clean_documents_moderate_results.csv")
df.to_csv(output_csv, index=False, encoding="utf-8-sig", sep=";")
...
# Save to CSV inside "results" folder
output_folder = "results_0.4_temperature/results_openai/monolingual/moderate_errors"
os.makedirs(output_folder, exist_ok=True)  # Create folder if it doesn't exist
